from .utils import date, run_sftp
...
class AuthKeys:
    ...
    def publish(self):
        ...
        entries = [
            fmt("# This file was generated by sshpush on {date}.")
        ]
        if self.include:
            entries += [
                '\n'.join([
    ...
    def verify(self):
        ...
        try:
            ...
            if test.read_text() == payload:
                comment('    %s: post update connection test successful.' % (self.server))
            else:
                warn('post update connection test failed.', culprit=self.server)
        except OSError as err:
            error(os_error(err))
